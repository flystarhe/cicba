...
def get_request_url(ak, sk):
    return "https://aip.baidubce.com/rest/2.0/ocr/v1/bankcard?access_token=" + get_token(ak, sk)


def for_image_data(img_data, detect_direction, ak, sk):
    try:
        request_url = get_request_url(ak, sk)
        params = {"image": img_data, "detect_direction": detect_direction}
...
def for_image_path(img_path, detect_direction, ak, sk):
    try:
        with open(img_path, "rb") as f:
            img_data = base64.b64encode(f.read())
        return for_image_data(img_data, detect_direction, ak, sk)
    except Exception as e:
        return 1, str(e)
